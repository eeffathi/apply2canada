...
function is_ic_product_listing( $query = null ) {
	...
	if ( empty( $query ) ) {
		...
		if ( ($query->is_post_type_archive( product_post_type_array() ) && !$query->is_search() && !is_ic_taxonomy_page( $query )) || is_home_archive( $query ) || is_custom_product_listing_page( $query ) ) {
			return true;
		}
	}
	return false;
...
function is_custom_product_listing_page( $query = null ) {
	...
	if ( !empty( $listing_id ) && is_ic_product_listing_enabled() ) {
		if ( empty( $query ) && is_page( $listing_id ) ) {
			return true;
		}
...
function is_ic_product_search( $query = null ) {
	...
	if ( ((empty( $query ) && is_search()) || (!empty( $query ) && $query->is_search()) ) && isset( $_GET[ 'post_type' ] ) && in_array( strval( $_GET[ 'post_type' ] ), product_post_type_array() ) ) {
		return true;
	}
	return false;
}
...
function is_ic_catalog_admin_page() {
	if ( is_admin() && function_exists( 'get_current_screen' ) ) {
		...
		if ( (isset( $screen->id ) && ic_string_contains( $screen->id, 'al_product' )) || is_ic_activation_notice() ) {
			return true;
		}
...
function is_ic_activation_notice() {
	...
	if ( $enable || isset( $_GET[ 'ic_catalog_activation_choice' ] ) ) {
		return true;
	}
	return false;
}
...
function is_ic_new_product_screen() {
	if ( is_admin() && function_exists( 'get_current_screen' ) ) {
		$screen = get_current_screen();
		if ( $screen->action == 'add' && is_ic_catalog_admin_page() ) {
			return true;
		}
	}
	return false;
...
function is_ic_product_list_admin_screen() {
	if ( is_admin() && function_exists( 'get_current_screen' ) ) {
		$screen = get_current_screen();
		if ( isset( $screen->id ) && ic_string_contains( $screen->id, 'edit-al_product' ) && is_ic_catalog_admin_page() ) {
			return true;
		}
	}
	return false;
...
function is_ic_shortcode_query() {
	...
	if ( (isset( $cat_shortcode_query ) && $cat_shortcode_query[ 'enable' ] == 'yes') || isset( $shortcode_query ) || !empty( $_POST[ 'ic_shortcode' ] ) ) {
		return true;
	} else {
		return false;
	}
...
function is_ic_integration_wizard_page( $check_shortcode_integration = true ) {
	if ( (((sample_product_id() && sample_product_id() == get_the_ID()) || !empty( $_GET[ 'test_advanced' ] )) && !is_archive()) && current_user_can( "manage_product_settings" ) && (!is_advanced_mode_forced( false ) || ic_is_woo_template_available() ) ) {
...
if ( !function_exists( 'ic_use_php_session' ) ) {
	...
	function ic_use_php_session() {
		if ( !defined( 'IC_USE_PHP_SESSIONS' ) ) {
			$return = false;
			if ( function_exists( 'session_start' ) && (!is_admin() || is_ic_front_ajax()) ) {
				$return = true;
			}
			if ( $return ) {
				define( 'IC_USE_PHP_SESSIONS', $return );
			}
			return $return;
		}
		return IC_USE_PHP_SESSIONS;
	}
...
if ( !function_exists( 'is_ic_shortcode_integration' ) ) {
	...
	function is_ic_shortcode_integration( $listing_id = null, $check_simple_mode = true ) {
		...
		$return = ic_get_global( 'is_ic_shortcode_integration' );
		if ( $return !== false ) {
			return $return;
		}
		//}
		...
		if ( !empty( $listing_id ) ) {
			if ( ic_has_listing_shortcode() || ($check_simple_mode && ic_catalog_theme_integration::get_real_integration_mode() === 'simple' && !is_advanced_mode_forced( false ) && !is_ic_integration_wizard_page( false )) ) {
				$return = apply_filters( 'is_ic_shortcode_integration', true );
				if ( !empty( $save_cache ) ) {
					ic_save_global( 'is_ic_shortcode_integration', $return );
				}
				return $return;
			}
		}
...
function ic_has_listing_shortcode() {
	$listing_id = intval( get_product_listing_id() );
	if ( !empty( $listing_id ) ) {
...
function ic_has_page_catalog_shortcode( $post ) {
	if ( has_shortcode( $post->post_content, 'show_product_catalog' ) || (function_exists( 'has_block' ) && has_block( 'ic-epc/show-catalog', $post )) ) {
		return true;
	} else {
		return false;
	}
