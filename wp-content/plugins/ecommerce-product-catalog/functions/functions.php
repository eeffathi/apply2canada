	...
	function ic_select_product( $first_option, $selected_value, $select_name, $class = null, $echo = 1, $attr = null,
		...
		if ( $product_count < 1000 ) {
			...
			foreach ( $pages as $product_id ) {
				...
				$select_box .= '<option class="id_' . $product_id . '" value="' . $product_id . '" ' . $selected . '>' . get_product_name( $product_id ) . '</option>';
			}
			$select_box .= '</select>';
		} else {
...
function get_product_catalog_page_title() {
	if ( is_ic_taxonomy_page() ) {
		...
		$title = __( 'Search Results for:', 'ecommerce-product-catalog' ) . ' <span class="ic-search-keyword">' . apply_filters( 'ic_search_keayword', strval( $_GET[ 's' ] ) ) . '</span>';
...
function get_related_categories( $product_id, $v_single_names = null, $taxonomy_name = 'al_product-cat' ) {
	...
	$terms			 = wp_get_post_terms( $product_id, $taxonomy_name, array( "fields" => "ids", "orderby" => "none" ) );
...
function product_breadcrumbs() {
	if ( get_integration_type() != 'simple' && !is_front_page() ) {
		...
		if ( !empty( $archive_names[ 'bread_home' ] ) ) {
			$count++;
			$bread .= apply_filters( 'product_breadcrumbs_home', '<span itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem" class="breadcrumbs-home"><a itemprop="item" href="' . $home_page . '"><span itemprop="name">' . $archive_names[ 'bread_home' ] . '</span></a><meta itemprop="position" content="' . $count . '" /></span> ' . $bread_divider . ' ' );
		}
		...
		if ( !empty( $parents ) && is_array( $parents ) ) {
			foreach ( $parents as $parent ) {
				if ( !empty( $parent ) ) {
					$count++;
					$additional .= ' ' . $bread_divider . ' <span itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">' . $parent . '<meta itemprop="position" content="' . $count . '" /></span>';
				}
			}
			if ( !empty( $additional ) ) {
				$bread .= $additional;
...
function all_ctalog_products_dropdown( $option_name, $first_option, $selected_value, $orderby = null, $order = null,
									   $all_option = false ) {
...
function ic_pre_get_products( $query ) {
	if ( ((!is_admin() && $query->is_main_query()) || is_ic_ajax()) && !isset( $_GET[ 'order' ] ) && ((!empty( $query->query[ 'post_type' ] ) && $query->query[ 'post_type' ] == 'al_product') || is_ic_product_listing( $query ) || is_ic_taxonomy_page( $query ) || is_ic_product_search( $query ) || is_home_archive( $query )) ) {
		...
		if ( (!empty( $query->query[ 'post_type' ] ) && ic_string_contains( $query->query[ 'post_type' ], 'al_product' ) && empty( $query->query[ 'name' ] )) || !empty( $query->query[ 'al_product-cat' ] ) ) {
...
add_action( 'ic_pre_get_products_only', 'set_product_order', 20 );
...
function set_product_order( $query ) {
	...
	if ( !isset( $_GET[ 'product_order' ] ) ) {
		if ( $archive_multiple_settings[ 'product_order' ] == 'product-name' && !is_ic_product_search( $query ) ) {
 ...
 * @param type $exclude
 */
function ic_get_to_hidden_field( $get, $exclude = '', $name = '', $value = '' ) {
	$fields = '';
	foreach ( $get as $key => $get_value ) {
		if ( (is_array( $exclude ) && !in_array( $key, $exclude )) || (!is_array( $exclude ) && $key !== $exclude) ) {
...
function ic_override_product_post_thumbnail( $metadata, $object_id, $meta_key ) {
	if ( isset( $meta_key ) && $meta_key == '_thumbnail_id' && is_ic_product( $object_id ) ) {
		$metadata = '';
	}
	return $metadata;
}
