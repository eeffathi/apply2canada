...
class ic_catalog_notices extends ic_activation_wizard {
	...
	function __construct( $run = false ) {
		if ( $run ) {
			...
			add_action( 'ic_catalog_admin_priority_notices', array( $this, 'admin_notices' ) );
	...
	function listing_page_info( $post ) {
		...
		if ( $listing_id == $post->ID ) {
			implecode_info( sprintf( __( 'This page is defined as main catalog listing. You can change this in %scatalog settings%s.', 'ecommerce-product-catalog' ), '<a href="' . admin_url( 'edit.php?post_type=al_product&page=product-settings.php' ) . '">', '</a>' ) );
			if ( !is_ic_shortcode_integration() ) {
				echo '<div></div>';
				implecode_info( sprintf( __( 'If you have any problems with your catalog display, please add the %s shortcode to the page content and save.', 'ecommerce-product-catalog' ), '[show_product_catalog]' ) );
			}
		}
	}
...
//$integration_type	 = get_integration_type();
		...
		if ( is_integration_mode_selected() && get_integration_type() == 'simple' ) {
			...
			<div class="notice notice-success is-dismissible ic-notice" data-ic_dismissible="notice-ic-catalog-welcome">
				<div class="squeezer">
					...
					<?php __( 'You can create your product listing page automatically or insert [show_product_catalog] shortcode on any of your existing pages.', 'ecommerce-product-catalog' ) ?>
	...
	function woocommerce_notice() {
		...
		if ( $this->show_woocommerce_notice() ) {
			...
			<div class="notice notice-info is-dismissible ic-notice" data-ic_dismissible="notice-ic-woocommerce-compat">
				<h4><?php echo sprintf( __( 'Hey! It looks like you have %1$s installed. You can use %2$s with or without %1$s.', 'ecommerce-product-catalog' ), 'WooCommerce', IC_CATALOG_PLUGIN_NAME ) ?> <?php _e( 'See the usage scenarios below:', 'ecommerce-product-catalog' ) ?></h4>
				<ul style="list-style: initial;list-style-position: inside;">
					<li><?php echo sprintf( __( '%1$s and %2$s separately for different product catalogs', 'ecommerce-product-catalog' ), 'WooCommerce', IC_CATALOG_PLUGIN_NAME ) ?> - <?php _e( "doesn't need any specific configuration", 'ecommerce-product-catalog' ) ?></li>
					<li><?php echo sprintf( __( '%1$s as an alternative to %2$s', 'ecommerce-product-catalog' ), IC_CATALOG_PLUGIN_NAME, 'WooCommerce' ) ?> - <?php echo sprintf( __( 'go to %1$s %3$simport screen%4$s to transfer all %2$s products to %1$s and remove %2$s after that', 'ecommerce-product-catalog' ), IC_CATALOG_PLUGIN_NAME, 'WooCommerce', '<a href="' . admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=product-settings&submenu=csv' ) . '">', '</a>' ) ?></li>
	...
	static function simple_mode_notice() {
		...
		if ( $listing_status !== 'publish' ) {
			...
			implecode_warning( sprintf( __( 'Disabled in simple mode. Add %1$s shortcode to your %2$smain catalog listing page%3$s to enable it.', 'ecommerce-product-catalog' ), '[show_product_catalog]', '<a href="' . product_listing_url() . '">', '</a>' ) );
		}
	}
