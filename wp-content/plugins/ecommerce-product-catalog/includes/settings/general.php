				...
				<form method="post" action="options.php">
					...
					if ( !is_advanced_mode_forced() || ic_is_woo_template_available() || is_ic_shortcode_integration() ) {
						...
						<div class="ic-important-settings">
							<?php implecode_info( __( 'This is the most important setting. The selected page slug will be included in each product url. You can select what will be shown on this page in the Main Listing Page section below. ', 'ecommerce-product-catalog' ) ) ?>
							<div></div>
							<span><?php _e( 'Main Catalog Page', 'ecommerce-product-catalog' ); ?>: </span><?php
							...
							<h3><?php
								$theme = wp_get_theme();
								echo __( 'Catalog Layout Integration with the theme', 'ecommerce-product-catalog' ) . ' (' . $theme->name . ')'
								?>
							</h3>
							<?php
							if ( ic_has_listing_shortcode() ) {
								implecode_info( __( 'You are currently using [show_product_catalog] on your product listing to integrate the catalog with the theme.', 'ecommerce-product-catalog' ) . '</p>' . '<p>' . __( 'If you have any problems with catalog layout you can remove the shortcode and use the theme integration wizard to fix it.', 'ecommerce-product-catalog' ) . '</p>' );
								...
								implecode_info(
								'<p>' . __( 'The shortcode integration is recommended for beginners.', 'ecommerce-product-catalog' ) . '</p>' .
								'<p>' . __( 'Place [show_product_catalog] in the text editor where you would like the product list to be displayed on your website.', 'ecommerce-product-catalog' ) . '</p>' .
								'<p>' . __( 'However, if you prefer to use the theme template files (which is a recommended choice for more advanced users) choose one of the options below.', 'ecommerce-product-catalog' ) . '</p>'
								);
								?>
								<table>
									<?php
									$descriptions[ 'simple' ]	 = '<strong>' . __( 'Simple Mode', 'ecommerce-product-catalog' ) . '</strong>' . ' - ' . __( 'will use your theme page template with catalog custom styling applied.', 'ecommerce-product-catalog' );
									$descriptions[ 'advanced' ]	 = '<strong>' . __( 'Advanced Mode', 'ecommerce-product-catalog' ) . '</strong>' . ' - ' . __( 'fully customizable layout with visual configuration wizard.', 'ecommerce-product-catalog' );
									...
									implecode_settings_radio( __( 'Layout mode', 'ecommerce-product-catalog' ), 'archive_multiple_settings[integration_type][' . $theme . ']', ic_catalog_theme_integration::get_real_integration_mode(), array( 'advanced' => $descriptions[ 'advanced' ], 'theme' => $descriptions[ 'theme' ], 'simple' => $descriptions[ 'simple' ] ), 1, sprintf( __( 'Choose Advanced Mode if you want to display catalog categories and search with %s layout.', 'ecommerce-product-catalog' ), IC_CATALOG_PLUGIN_NAME ), '<br>', "integration-mode-selection" );
									?></table>
								<div class="simple_mode_settings">
									...
									'<p>' . __( 'If you want to switch to advanced mode easily, please insert the [show_product_catalog] shortcode on your product listing page.', 'ecommerce-product-catalog' ) . '</p>'
									)
									?>
								</div>
								<div class="advanced_mode_settings">
									...
									'<p>' . __( 'If you have any problems with the integration please insert the [show_product_catalog] shortcode on any page to display the catalog.', 'ecommerce-product-catalog' ) . '</p>'
									);
									...
									<table class="advanced_mode_settings_hidden">
										<tr><td style="min-width: 250px"></td><td></td></tr>
										<?php
...
function get_product_listing_id() {
	...
	if ( !empty( $cache_key ) ) {
		wp_cache_set( $cache_key, $return_listing_id, 'implecode' );
	}
