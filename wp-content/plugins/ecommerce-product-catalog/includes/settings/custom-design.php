...
function ic_product_page_design_settings() {
	...
	implecode_upload_image( __( 'Default Image', 'ecommerce-product-catalog' ), 'default_product_thumbnail', get_default_product_image_src() )
	?>
...
function get_modern_grid_settings() {
	$settings = wp_parse_args( get_option( 'modern_grid_settings' ), apply_filters( 'ic_modern_grid_defaults', array( 'attributes' => 0, 'per-row' => 2, 'per-row-categories' => 2, 'attributes_num' => 6 ) ) );
	return $settings;
}
