...
if ( !function_exists( 'echo_ic_setting' ) ) {
	...
	function implecode_settings_radio( $option_label, $option_name, $option_value, $elements = array(), $echo = 1,
		...
		foreach ( $elements as $key => $element ) {
			...
			$return .= '<span style="display:table-cell"><input type="radio" class="' . $class . '" id="' . $option_name . '_' . $key . '" name="' . $option_name . '" value="' . $key . '"' . checked( $key, $option_value, 0 ) . '/></span>' . '<label for="' . $option_name . '_' . $key . '" style="display: table-cell">' . $element . '</label><div></div>';
		}
		$return	 .= '</td>';
		$return	 .= '</tr>';
	...
	function implecode_settings_number( $option_label, $option_name, $option_value, $unit, $echo = 1, $step = 1,
		...
		$return	 .= '<input type="number" step="' . $step . '" ' . $min . ' ' . $max . ' class="' . $class . '" name="' . $option_name . '" value="' . floatval( $option_value ) . '"' . $attr . ' />' . $unit . '</td>';
		$return	 .= '</tr>';
