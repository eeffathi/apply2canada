...
function implecode_custom_csv_menu() {
	?>
	<a id="csv-settings" class="element" href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=product-settings&submenu=csv' ) ?>"><?php _e( 'Import / Export', 'ecommerce-product-catalog' ); ?></a>
	<?php
}
		...
		foreach ( $post_meta as $name => $value ) {
			...
			if ( is_array( $value ) ) {
				foreach ( $value as $val ) {
					add_post_meta( $target_post_id, $name, $val );
				}
			} else {
				add_post_meta( $target_post_id, $name, $value );
			}
		}
	}
