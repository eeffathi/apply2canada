	...
	final class eCommerce_Product_Catalog {
		...
		public function implecode_run_admin_styles() {
			...
			if ( is_ic_admin_page() ) {
				...
				wp_localize_script( 'al_product_admin-scripts', 'ic_catalog', array(
		...
		public function implecode_enqueue_styles() {
			...
			wp_localize_script( 'al_product_scripts', 'product_object', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ), 'post_id' => get_the_ID(), 'lightbox_settings' => $colorbox_set, 'filter_button_label' => __( 'Filter', 'ecommerce-product-catalog' ), 'design_schemes' => design_schemes( 'box', 0 ) ) );
			if ( function_exists( 'is_ic_attributes_size_enabled' ) && is_ic_attributes_size_enabled() ) {
