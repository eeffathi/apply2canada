 ...
 * @author 		impleCode
 */
add_action( 'admin_product_details', 'ic_price_metabox', 5, 2 );
...
function ic_price_metabox( $product_details, $product_id ) {
	if ( is_ic_price_enabled() ) {
		...
		$product_details .= apply_filters( 'admin_price_table', '<table><tr><td class="label-column">' . __( 'Price', 'ecommerce-product-catalog' ) . ':</td><td class="price-column"><input type="text" title="' . sprintf( __( 'Example price format: %s or %s', 'ecommerce-product-catalog' ), price_format( '1587.89', 1, 0 ), '1587' . $set[ 'dec_sep' ] . '89' ) . ' (' . __( 'you can change it in product settings', 'ecommerce-product-catalog' ) . ')" pattern="^(([1-9](\\d*|\\d{0,2}(' . $set[ 'th_sep' ] . '\\d{3})*))|0)(' . $set[ 'dec_sep' ] . '\\d{1,2})?$" name="_price" value="' . $formatted_price . '" class="widefat" /></td><td>' . product_currency() . '</td></tr></table>', $product_id );
