...
function al_product_attributes( $post, $args = null ) {
	...
	implecode_info( __( 'Only attributes with values set will be shown on product page.', 'ecommerce-product-catalog' ) . ' ' . sprintf( __( 'See <a target="_blank" href="%s">docs</a>.', 'ecommerce-product-catalog' ), 'https://implecode.com/docs/ecommerce-product-catalog/product-attributes/?cam=catalog-add-page-box&key=product-attributes' ) );
	...
	for ( $i = 1; $i <= $attributes_number; $i++ ) {
		...
		$attribute_value_field	 = '<input ' . $autocomplete . ' class="ic_autocomplete attribute-value" type="text" name="' . $names[ 'value' ] . $i . '" value="' . esc_html( $field_value ) . '" />';
		?>
		<tr>
			<td class="attributes-label-column"><input <?php echo $label_autocomplete ?> class="ic_autocomplete attribute-label" type="text" name="<?php echo $names[ 'label' ] . $i ?>" value="<?php echo esc_html( $attributes_label_option_field ) ?>"/></td>
...
function ic_size_metabox( $product_details, $product_id, $field_name = null, $size = null ) {
	if ( is_ic_attributes_size_enabled() ) {
		...
		if ( !empty( $field_names ) ) {
			...
			$fields	 = '<input placeholder="' . _x( 'L', 'Length', 'ecommerce-product-catalog' ) . '" style="max-width: 25%;" type="text" name="' . $field_name[ 0 ] . '" value="' . $size[ 'length' ] . '" class="widefat" />';
			$fields	 .= 'x <input placeholder="' . _x( 'W', 'Width', 'ecommerce-product-catalog' ) . '" style="max-width: 25%;" type="text" name="' . $field_name[ 1 ] . '" value="' . $size[ 'width' ] . '" class="widefat" />';
			$fields	 .= 'x <input placeholder="' . _x( 'H', 'Height', 'ecommerce-product-catalog' ) . '" style="max-width: 25%;" type="text" name="' . $field_name[ 2 ] . '" value="' . $size[ 'height' ] . '" class="widefat" />';
		}
