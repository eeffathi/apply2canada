	...
	static function theme_integration_wizard( $atts ) {
		...
		if ( is_ic_integration_wizard_page() ) {
			...
			if ( !isset( $_GET[ 'test_advanced' ] ) ) {
				...
				if ( is_ic_shortcode_integration( null, false ) ) {
					$box_content .= '<p class="initial-description">' . __( 'Would you like to remove the shortcode integration and try the configuration wizard?', 'ecommerce-product-catalog' ) . '</p><p><strong>' . sprintf( __( "If something goes wrong you will have to add the %s shortcode to the product listing again.", 'ecommerce-product-catalog' ), '[[show_product_catalog]]' ) . '</strong></p><p><strong>' . __( 'Do it only if the catalog layout doesn\'t match your theme or if some unwanted elements from the theme are displayed on catalog pages.', 'ecommerce-product-catalog' ) . '</strong></p>';
					$box_content .= '<p class="wp-core-ui" style="margin-top: 20px;"><a href="' . add_query_arg( 'remove_shortcode_integration', '1' ) . '" class="button-primary">' . __( 'Remove the Shortcode and Start the Wizard', 'ecommerce-product-catalog' ) . '</a></p>';
				} else {
...
//$box_content .= '<p>' . __( 'Please use the buttons below to let the script know if the Automatic Advanced Integration is done right.', 'ecommerce-product-catalog' ) . '</p>';
	...
	margin-left: -23px;}</style>';
				...
				$box_content .= '<li>' . sprintf( __( 'Use %s shortcode on your product listing page.', 'ecommerce-product-catalog' ), '[[show_product_catalog]]' ) . '</li>';
