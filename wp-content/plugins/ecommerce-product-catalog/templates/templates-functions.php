	...
	if ( $page != '' && !is_ic_shortcode_integration() ) {
		if ( get_integration_type() != 'simple' ) {
			if ( ic_has_page_catalog_shortcode( $page ) ) {
				$page->post_content = str_replace( '[show_product_catalog]', '', $page->post_content );
			}
...
function show_products_outside_loop( $atts, $content = null ) {
	...
	while ( $shortcode_query->have_posts() ) : $shortcode_query->the_post();
		global $post;
		ic_save_global( 'product_id', $post->ID );
		$i++;
...
function more_products() {
	...
	if ( !empty( $y_query->posts ) && is_array( $y_query->posts ) ) {
...
function exclude_products_from_child_cat( $query ) {
	if ( !is_product_filter_active( 'product_category' ) && empty( $_GET[ 'product_category' ] ) && (!is_admin() || is_ic_ajax()) && (is_ic_ajax() || $query->is_main_query()) && is_ic_taxonomy_page( $query ) && is_ic_only_main_cats( $query ) ) {
...
function ic_get_product_categories( $product_id ) {
	$taxonomy	 = apply_filters( 'ic_cat_related_products_tax', get_current_screen_tax() );
...
function ic_product_listing_products( $archive_template, $multiple_settings ) {
	...
	if ( /* (!is_ic_only_main_cats() || is_search() || is_product_filters_active() || is_ic_ajax() ) && */more_products() ) {
		...
		echo '<div class="product-list"><p>' . apply_filters( 'ic_catalog_not_found_message', $notfound_text ) . '</p></div>';
		product_search_form();
	} else if ( is_product_filters_active() && !more_products() ) {
		show_product_sort_bar();
		$notfound_filters_text = sprintf( __( 'Sorry, but nothing matched your search terms. Please try again with some different options or %sreset filters%s.', 'ecommerce-product-catalog' ), '<a href="' . get_filters_bar_reset_url() . '">', '</a>' );
		echo '<div class="product-list"><p>' . apply_filters( 'ic_catalog_not_found_filters_message', $notfound_filters_text ) . '</p></div>';
	} else if ( !more_products() && (!is_ic_only_main_cats() || (is_ic_taxonomy_page() && !has_category_children())) ) {
